...
N = 100  # 100个测试点
rootDir = r'D:\Users\Wxss\01Project\01VVC\03Data\01COA\20260509'
subDirList = next(os.walk(rootDir))[1]
SNPtn = r"081900\d{2}-00[12]"
...
for subDir in subDirList[:]:
  if re.search("figure", subDir):
    subDirList.remove(subDir)
subDir = subDirList[7]
dataDir = os.path.join(rootDir, subDir)
figDir = os.path.join(rootDir, rf'{re.search(SNPtn, dataDir).group(0)} figure（循环后）')
...
for i in range(dfStepAllLen):  # 每一个测试文件，数据长度不一样，无法使用定长数据读数
  if dfStepAll.iloc[i, 0] > CapRT:
    ...
    # 挑出步进容值变化异常的曲线
    # if dfStepAll['CW(pF)'][(i - k):i].max() > 0.5:
    #   print(dfStepAll.iloc[i-k, 5])
